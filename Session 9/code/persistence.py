...
def _atomic_write(path: Path, data: bytes | str) -> None:
    ...
    if isinstance(data, bytes):
        ...
        # Text is ALWAYS written UTF-8. On Windows the default text encoding
        # is cp1252, which cannot encode '₹' (U+20B9) and many other non
        # Latin-1 glyphs that appear in real queries and answers ("laptops
        # under ₹80,000"). Without the explicit encoding the first write of
        # such a run dies with UnicodeEncodeError before the graph runs.
