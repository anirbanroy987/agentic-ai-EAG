...
_GEMINI_DROP_KEYS = {
    "additionalProperties", "$schema", "title", "definitions", "$defs",
    "examples", "default", "discriminator", "$id", "$comment",
    "readOnly", "writeOnly", "patternProperties",
}


def _json_type_of(v) -> Optional[str]:
...
def _gemini_clean_schema(schema: dict) -> dict:
    """
    Convert an arbitrary JSON-Schema (including Pydantic v2 output) into the
    OpenAPI-3 subset Gemini's responseSchema accepts.

    Pydantic emits `$ref`/`$defs` for nested models, `const` for single-value
    Literals, `anyOf:[T, {type:null}]` for Optionals, and `allOf:[{$ref}]`
    wrappers — none of which Gemini accepts. We:
      - inline every local `$ref` from `$defs`/`definitions` (cycle-guarded),
      - rewrite `const: X`            → `enum: [X]` (+ inferred `type`),
      - collapse `anyOf/oneOf` of one real type + null → that type + `nullable`,
      - merge `allOf` member subschemas into the parent,
      - drop keys Gemini rejects (`additionalProperties`, `title`, ...).
    """
    ...
    def walk(node: Any, seen: frozenset) -> Any:
        if isinstance(node, list):
            return [walk(x, seen) for x in node]
        if not isinstance(node, dict):
            return node

        # $ref → inline the target, then overlay sibling keys.
        if "$ref" in node:
            ref = node["$ref"]
            if ref in seen:
                return {"type": "object"}
            merged = walk(resolve_ref(ref), seen | {ref})
            ...
            for k, v in node.items():
                if k == "$ref":
                    continue
                merged[k] = walk(v, seen)
            return _gemini_clean_schema(merged) if "$ref" in merged else merged
        ...
        for k, v in node.items():
            ...
            if k == "const":
                out["enum"] = [v]
                out.setdefault("type", _json_type_of(v) or "string")
                continue
            if k == "allOf" and isinstance(v, list):
                for member in v:
                    wm = walk(member, seen)
                    if isinstance(wm, dict):
                        for mk, mv in wm.items():
                            out.setdefault(mk, mv)
                continue
            if k in ("anyOf", "oneOf") and isinstance(v, list):
                variants = [walk(x, seen) for x in v if isinstance(x, dict)]
                has_null = any(
                    x.get("type") == "null" for x in variants
                )
                non_null = [x for x in variants if x.get("type") != "null"]
                if len(non_null) == 1:
                    only = non_null[0]
                    if has_null:
                        only = {**only, "nullable": True}
                    for mk, mv in only.items():
                        out.setdefault(mk, mv)
                else:
                    out[k] = non_null or variants
                    if has_null:
                        out["nullable"] = True
                continue
            out[k] = walk(v, seen)
...
def _coerce_obj(v):
    # Gemini's function_response.response must be a protobuf Struct (a JSON
    # object). Anything that isn't a dict — scalars, lists, JSON that decodes
    # to a scalar like "30" — must be wrapped or Gemini returns HTTP 400.
    if isinstance(v, dict):
        return v
    if isinstance(v, list):
        return {"result": v}
