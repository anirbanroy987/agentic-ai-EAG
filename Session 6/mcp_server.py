...
# Force UTF-8 on stdio so Rich/crawl4ai output doesn't UnicodeEncodeError on
# Windows consoles (cp1252 default). The fd-level dup2 below only changes
# *where* the bytes go — the encoding crash happens earlier, inside Python's
# TextIOWrapper. errors="replace" so any genuinely unmappable character
# becomes '?' instead of killing the tool call.
for _stream in (sys.stdout, sys.stderr):
    try:
        _stream.reconfigure(encoding="utf-8", errors="replace")
    except (AttributeError, OSError):
        # Non-text streams (e.g. when stdout is piped elsewhere) — skip.
        pass

# crawl4ai prints an [INIT] banner via Rich at IMPORT time. Under MCP stdio
# that goes straight onto the JSON-RPC channel (fd 1) and desyncs every
# subsequent tool call — the next call_tool reply lands behind garbage and
# the client times out. The function-level dup2 below fires *after* this
# import, so it can't save us; import crawl4ai here, under the redirect,
# once. Module-load cost: ~1-2s, paid once.
...
async def _crawl4ai_fetch(url: str) -> dict:
    # The class import is now at module load (see the dup2 block at the top),
    # so the [INIT] banner has already been absorbed. The runtime [FETCH] /
    # [SCRAPE] / [COMPLETE] markers still fire from arun() — keep the
    # file-descriptor redirect around it so they don't corrupt the JSON-RPC
    # stream either. contextlib.redirect_stdout doesn't catch Rich's writes.
    saved_fd = os.dup(1)
    os.dup2(2, 1)
    try:
