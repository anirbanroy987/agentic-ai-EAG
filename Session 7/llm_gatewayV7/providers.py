...
class OpenAICompatProvider(BaseProvider):
    ...
    def _ensure_json_mention(self, body):
        """Groq (and some other OpenAI-compatible backends) reject
        `response_format` of type `json_object` unless the word 'json' appears
        somewhere in the messages. When the gateway downgrades a strict
        json_schema request to json_object for these providers, that word is
        often absent, producing an HTTP 400 that aborts failover. Inject a
        minimal system instruction so the constraint is always satisfied."""
        msgs = body.get("messages") or []
        if any("json" in (m.get("content") or "").lower() for m in msgs):
            return
        msgs.insert(0, {"role": "system", "content": "Respond with a single valid JSON object."})
        body["messages"] = msgs
    ...
    def _apply_response_format(self, body, response_format):
        ...
        if rf.get("type") == "json_schema" and rf.get("schema"):
            ...
            # If this provider can't honor json_schema and the gateway falls
            # back to json_object (below), the 'json' mention must already be
            # present — inject it preemptively so the retry succeeds.
            self._ensure_json_mention(body)
