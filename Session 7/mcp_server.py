...
# '→' (→). crawl4ai/Rich writes progress banners through these streams
# during fetch_url, and the encode crash propagated out as a tool error.
# Force UTF-8 (errors="replace" as a belt-and-braces guard) so tool execution
# never dies on a non-Latin-1 byte. JSON-RPC over stdout stays ASCII-safe.
