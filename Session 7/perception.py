...
def observe(
    ...
    for i, d in enumerate(raw_goals):
        ...
        # Complement of the downgrade guard: if the model FAILED to mark a
        # goal done but RUN HISTORY already shows a substantive answer
        # (kind=="answer") for this goal's id, force it done. An "answer"
        # event is only written when Decision actually produced a final reply
        # for that goal, so closing it is correct for any goal type. Without
        # this, a synthesis goal can loop forever — Decision re-emits the same
        # answer each iteration because the goal never closes and all_done
        # never trips. Termination must not depend on the LLM reliably
        # volunteering done=true (model-swap exposed exactly this).
