...
# ── Windows asyncio Proactor cleanup-noise silencer ────────────────────────
# When this module finishes its `asyncio.run(...)` call the event loop
# closes — but on Windows the Proactor event loop's subprocess pipe
# transports outlive that closure because Python finalises them later
# via GC. Their `__del__` methods then try to call back into the (now
# closed) loop and raise:
#
#     ResourceWarning: unclosed transport ...
#     ValueError: I/O operation on closed pipe
#     RuntimeError: Event loop is closed
#
# These are emitted via `sys.unraisablehook` during interpreter shutdown.
# They are HARMLESS — the run has already completed and persisted its
# results to disk by the time these fire — but they spam ~80 lines of
# traceback into the dashboard's Execution-logs tab and the terminal.
#
# Fix: install an unraisable hook that swallows exactly this trio of
# errors and forwards everything else to the default handler. We do
# NOT monkey-patch `_ProactorBasePipeTransport.__del__` (the other
# common workaround) because that touches a private asyncio class
# whose signature has changed twice between 3.10 and 3.12.
if sys.platform == "win32":
    _default_unraisable = sys.unraisablehook

    def _silence_proactor_cleanup(unraisable):  # noqa: ANN001
        exc = unraisable.exc_value
        msg = str(exc) if exc is not None else ""
        # Match the three known finalisation errors and only those.
        if isinstance(exc, (RuntimeError, ValueError, ResourceWarning)) and (
            "Event loop is closed" in msg
            or "closed pipe" in msg
            or "I/O operation on closed pipe" in msg
            or "unclosed transport" in msg
        ):
            return
        _default_unraisable(unraisable)

    sys.unraisablehook = _silence_proactor_cleanup

    # Also belt-and-braces: tell asyncio's default Proactor loop to use
    # the WindowsProactorEventLoopPolicy explicitly. The default is
    # already Proactor on 3.8+, but pinning it documents intent and
    # avoids a 3.13 deprecation message about implicit policy.
