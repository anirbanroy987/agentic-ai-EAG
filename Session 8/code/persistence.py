...
def _atomic_write(path: Path, data: bytes | str) -> None:
    """Atomic write — write to a .tmp sibling, then `os.replace` over the
    target so a SIGKILL mid-write cannot corrupt the last good snapshot.

    Text writes are FORCED to UTF-8. Default text-mode `open()` on Windows
    uses the system codepage (cp1252) which cannot encode currency symbols
    (₹, €), em-dashes, smart quotes, or any non-Latin script — and LLM
    output contains all of those. Without an explicit encoding here a
    finance answer with a ₹ in it blows up the entire run with
    UnicodeEncodeError. The MCP server already reads files as utf-8; the
    writer side must match.
    """
...
class SessionStore:
    ...
    def read_query(self) -> str:
        if not self.query_path.exists():
            return ""
        # encoding="utf-8" mirrors _atomic_write: on Windows the default text
        # encoding is cp1252 and a query containing ₹/em-dash/non-Latin script
        # crashes the read.
