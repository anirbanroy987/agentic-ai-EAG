...
def _load() -> list[MemoryItem]:
    if not STATE_PATH.exists():
        return []
    # encoding="utf-8" — indexed finance chunks contain ₹ and other non-Latin
    # characters; Windows default cp1252 crashes on read.
    raw = json.loads(STATE_PATH.read_text(encoding="utf-8"))
    return [MemoryItem.model_validate(r) for r in raw]


def _save(items: list[MemoryItem]) -> None:
    # ensure_ascii=False keeps ₹ etc. as their natural characters in the file;
    # encoding="utf-8" on the write side is the symmetric requirement.
    STATE_PATH.write_text(
        json.dumps([i.model_dump(mode="json") for i in items], indent=2, ensure_ascii=False),
        encoding="utf-8",
    )
