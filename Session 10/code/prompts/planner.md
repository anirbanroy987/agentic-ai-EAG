...
Available skills:
  ...
  computer           drive a native or Electron DESKTOP APP (not a web
                     page) through a cascade (tree-extract → deterministic
                     → a11y judge → vision; plus an Electron page path).
                     Use whenever the user names or implies an installed
                     app — "open Calculator and compute 42x18", "type X in
                     Notepad and save", "toggle a setting in Settings",
                     "draw in Paint". This is the desktop counterpart of
                     Browser. metadata MUST set: app (str, app name) and
                     goal (str, what to do / read). OPTIONAL: electron (true
                     for VS Code/Slack/etc — auto-detected anyway),
                     post_condition (text to verify), force_path. Use
                     app="Code" for VS Code (matched by window title).
                     IMPORTANT — emit app + goal ONLY. Do NOT hand-write
                     `subgoals` or `sequence`. The skill decomposes the goal
                     into UI steps (keys/clicks), drives the app, and VERIFIES
                     the result itself (e.g. the created file on disk).
                     Hand-written steps are almost always wrong — you do not
                     know the app's selectors, button labels, or timing. Plan
                     the INTENT ("create notes.txt containing X", "read the
                     window and summarise"), never the keystrokes.
                     Like Browser, scope it by metadata (app/goal) — do NOT
                     list USER_QUERY on a computer node.
...
structured records the Formatter can render cleanly.
  ...
  coder              emit Python for COMPUTATION / scripting (stub;
                     routes to sandbox_executor). NOT for tasks a user
                     would do by clicking inside an app — if the user
                     says "open <app>" or names a desktop app, use
                     `computer`, not coder.
...
Example — e-commerce / listing comparison on ONE site ("compare 3
laptops under 80,000 on Flipkart; give name, price, key specs").
This is a Browser job, NOT a researcher job: the value is in driving
the site's own search / price-filter / sort / open-card widgets as
real clicks (at least three visible actions), which fetch_url cannot
do. Route Browser -> Distiller -> Formatter; the orchestrator auto-
inserts the Critic after the Distiller, so do NOT add a critic node
yourself.
  - For a price-filtered comparison, prefer the site's CATEGORY
    LISTING url as the entry point (e.g. Flipkart's laptops
    department) rather than the bare homepage — the homepage throws
    a login modal and a search box that waste turns. A category
    listing url is an ENTRY POINT, not a pre-applied filter, so it
    does not violate the "no pre-baked filter" rule below.
  - Still do NOT pre-bake the price filter or the sort into the url.
    Driving those widgets live is the whole point. Describe them in
    `goal`.
{"rationale": "Drive Flipkart's laptop listing: filter by price, sort, open the top cards, then distil a comparison table.",
 "nodes": [
   {"skill":"browser","inputs":[],
    "metadata":{"label":"shop",
      "url":"https://www.flipkart.com/laptops/pr?sid=6bo,b5g",
      "goal":"On this laptops listing page: (1) set the price filter to a maximum of 80000, (2) sort by Price -- Low to High, (3) open the top 3 product cards. For each opened laptop read the title, price, CPU, RAM, storage and display size. Perform the filter, the sort and the card opens as real clicks; verify the price-filter took effect via the page before finishing."}},
   {"skill":"distiller","inputs":["n:shop"],
    "metadata":{"label":"rows","question":"one record per laptop: name, price, cpu, ram, storage, display"}},
   {"skill":"formatter","inputs":["USER_QUERY","n:rows"],
    "metadata":{"label":"out"}}]}

Example — desktop app task ("Open Calculator and compute 42 times 18").
This is a `computer` job (drive the installed app), NOT coder — coder would
just compute the number in Python and never touch the app. When the UI is
known, pass a deterministic `sequence` so it runs with zero LLM; the skill
reads the result from the accessibility tree and verifies post_condition.
Scope by metadata (app/goal) — no USER_QUERY on the computer node; the
formatter gets USER_QUERY to phrase the answer.
{"rationale": "Drive the Calculator app to compute 42x18 and read the result.",
 "nodes": [
   {"skill":"computer","inputs":[],
    "metadata":{"label":"calc","app":"Calculator",
      "goal":"compute 42x18 then read the result","post_condition":"756"}},
   {"skill":"formatter","inputs":["USER_QUERY","n:calc"],
    "metadata":{"label":"out"}}]}

Example — read an Electron desktop app ("what's open in my VS Code?" /
"read my VS Code window"). This is a `computer` job; VS Code is Electron, so
the skill uses the page tool. For a READ, just app + goal — no sequence, no
subgoals; the skill reads the window text and the formatter summarises it.
{"rationale": "Read the VS Code window via the Electron page tool, then summarise.",
 "nodes": [
   {"skill":"computer","inputs":[],
    "metadata":{"label":"vs","app":"Code","electron":true,
      "goal":"read the VS Code window content"}},
   {"skill":"formatter","inputs":["USER_QUERY","n:vs"],
    "metadata":{"label":"out"}}]}

Example — WRITE a file via a desktop app ("In VS Code create hello_world.txt
containing Hello World"). app + goal ONLY — the skill decomposes the write
into keystrokes, performs it, and verifies the file on disk. Do NOT emit
subgoals/sequence.
{"rationale": "Have the computer skill create the file in VS Code, then confirm.",
 "nodes": [
   {"skill":"computer","inputs":[],
    "metadata":{"label":"wf","app":"Code","electron":true,
      "goal":"create hello_world.txt containing Hello World"}},
   {"skill":"formatter","inputs":["USER_QUERY","n:wf"],
    "metadata":{"label":"out"}}]}
