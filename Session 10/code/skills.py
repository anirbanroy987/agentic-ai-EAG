...
async def run_skill(skill: Skill, node_id: str, graph_nodes,
    ...
    if skill.name == "computer":
        ...
        # cascade (tree-extract → deterministic → a11y judge → vision, plus an
        # Electron `page` path) over a running cua-driver daemon, and routes
        # each layer's gateway call internally — so we bypass render_prompt and
        # the gateway-chat dispatch and hand off to ComputerSkill.run(NodeSpec).
        node_dict = graph_nodes[node_id]
        node_spec = NodeSpec(
            skill="computer",
            inputs=node_dict.get("inputs") or [],
