...
def _find_gateway_dir() -> Path:
    ...
    The previous hard-coded `parents[2]` assumed a fixed nesting depth and
    broke the moment the package was checked out one level shallower
    (e.g. `Session 9/code/` with the gateway at `Session 9/llm_gatewayV9/`
    resolves `parents[2]` to the repo root, where no gateway exists — so
    the in-process client import silently fell back to `LLM = None` and the
    first skill call died with `'NoneType' object is not callable`). Walking
    up and matching on the presence of `client.py` is layout-agnostic.
    """
    here = Path(__file__).resolve()
    for parent in here.parents:
        cand = parent / "llm_gatewayV9"
        if (cand / "client.py").exists():
            return cand
    # Fall back to the original assumption so the error messages below still
    # point somewhere sensible when the gateway genuinely isn't checked out.
    return here.parents[2] / "llm_gatewayV9"
