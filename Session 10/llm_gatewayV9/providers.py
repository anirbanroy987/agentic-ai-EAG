...
VISION_MODEL_HINTS = (
    ...
    "qwen2vl", "qwen2.5vl", "qwen3vl",   # Ollama tags drop the hyphen
    "llama-3.2-11b-vision", "llama-3.2-90b-vision",
    "llama-4",  # Llama 4 Scout/Maverick are multimodal (Groq LPU, etc.)
...
def build_providers(cache_store):
    ...
    if k := os.getenv("GROQ_API_KEY"):
        ...
        # V9: a SECOND groq instance pinned to a multimodal model (Llama 4
        # Scout/Maverick) so the fast LPU can serve /v1/vision while the text
        # `groq` instance (e.g. critic) stays on its 70B model. Same API key →
        # shares Groq's real account rate limits; tracked as its own bucket.
        if vm := os.getenv("GROQ_VISION_MODEL"):
            out["groqvision"] = GroqProvider(k, vm)
            out["groqvision"].name = "groqvision"
