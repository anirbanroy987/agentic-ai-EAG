...
def _index(exclude_id: str | None = None) -> VectorIndex:
    ...
    `exclude_id` lets `_persist_item` preserve memory.json-first ordering
    (the source-of-truth discipline) without double-adding a vector: the new
    item is written to memory.json BEFORE the index is touched, so the cold
    rebuild below would otherwise pick it up here AND again via the explicit
    `idx.add` in `_persist_item`. Skipping it during the rebuild keeps the
    write order intact while adding each vector exactly once.
    """
...
def _persist_item(item: MemoryItem) -> MemoryItem:
    ...
    Source-of-truth ordering: memory.json is written FIRST, then FAISS. The
    JSON store is authoritative; the index is a derived artifact that can be
    rebuilt from it. To avoid the cold-start double-add (the rebuild in
    `_index()` reading the just-saved item AND the explicit `idx.add` here
    both inserting it — the 26-vs-25 off-by-one), the rebuild is told to skip
    this item's id via `exclude_id`. Net effect: write order preserved, each
    vector added exactly once.
    """
    items = _load()
    items.append(item)
    _save(items)
    if item.embedding is not None and item.kind in _EMBEDDABLE_KINDS:
        idx = _index(exclude_id=item.id)
        idx.add(item.id, item.embedding)
        idx.persist()
    return item
